...
class OnboardingFlow {
    constructor(logger, telemetry, config, context) {
        this.logger = logger;
    ...
    async start() {
        ...
        // Show initial prompt
        const choice = await vscode.window.showInformationMessage('Welcome to Adaptive Tests! Would you like a quick tour of the features?', { modal: false }, 'Start Tour (2 min)', 'Skip Tour', 'Remind Me Later');
        if (choice === 'Skip Tour') {
    ...
    async runStep(stepIndex) {
        ...
        const choice = await vscode.window.showInformationMessage(`${step.title}\n\n${step.description}\n\n${progress}`, { modal: true }, ...buttons);
        // Execute step action
        if (step.action && choice !== 'Skip') {
            try {
                ...
                this.logger.error('Onboarding step failed', error);
    ...
    async showWelcomeAnimation() {
        ...
        const panel = vscode.window.createWebviewPanel('adaptiveTestsWelcome', 'Welcome to Adaptive Tests', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async createSampleWorkspace() {
        const workspacePath = path.join(require('os').tmpdir(), 'adaptive-tests-sample', Date.now().toString());
...
Happy testing! 🚀`
    ...
    async demonstrateDiscovery() {
        ...
        const panel = global.discoveryLensPanel;
        if (panel) {
    ...
    async configureAIFeatures() {
        const enableAI = await vscode.window.showQuickPick(['Yes, enable AI features', 'Maybe later'], {
    ...
    async waitForUserInteraction(timeout = 5000) {
    ...
    async completeOnboarding() {
        ...
        vscode.window.showInformationMessage('🎉 Congratulations! You\'ve completed the Adaptive Tests onboarding!', 'Open Discovery Lens', 'View Documentation').then(choice => {
    ...
    shouldRepeatOnboarding() {
        // Check if major version changed
        const lastVersion = this.context.globalState.get('lastVersion');
        const currentVersion = this.context.extension.packageJSON.version;
        if (lastVersion && lastVersion.split('.')[0] !== currentVersion.split('.')[0]) {
            return true; // Major version change
        }
        // Check if reminder time has passed
        const remindAt = this.context.globalState.get('onboarding.remindAt');
